...
const defaultBaseURL = "https://www.telelistas.net/busca"
...
func (s *Source) Name() string             { return "TeleListas" }
func (s *Source) Tier() sources.SourceTier { return sources.TierCrowdsource }
func (s *Source) Jurisdiction() []string   { return []string{"BR"} }
...
func (s *Source) DryRun(ctx context.Context, e164 string) error {
	...
	if !strings.HasPrefix(e164, "+55") {
		return fmt.Errorf("skipped: TeleListas supports +55 numbers only")
	}
	return nil
}
func (s *Source) Fetch(ctx context.Context, e164 string) ([]correlator.PIIClaim, error) {
	if s.limiter != nil {
		if err := s.limiter.Wait(ctx, "telelistas.net"); err != nil {
			return nil, err
		}
	}
	body, err := sources.Get(ctx, s.client, sources.BuildURL(s.baseURL, nil, map[string]string{"q": national(e164, "+55")}), map[string]string{"Accept": "text/html,application/xhtml+xml"})
	if err != nil {
		return nil, nil
	}
...
func parseHTML(body []byte) (string, string) {
	...
	if strings.Contains(strings.ToLower(name), "telelistas") {
		name = ""
	}
	return name, trim(extractAfter(clean, "Endereço"))
}
